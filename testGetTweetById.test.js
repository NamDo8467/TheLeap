const request = require("supertest")
const express = require("express")
const session = require("express-session")
const { mongoose, server, app } = require("./index")

afterEach(async () => {
	server.close()
})
afterAll(() => {
	return mongoose.connection.close()
})

const mockApp = express()
const TESTING_TWEET_ID = "638a8d5a20a837a95201957b"

/* TEST GET TWEET BY ID */
describe("GET /api/tweet/_id/:id", () => {
	beforeEach(() => {
		const TIME = 1000 * 60 * 60
		mockApp.use(
			session({
				secret: "iamnamdo1234567",
				saveUninitialized: true,
				cookie: { maxAge: TIME, sameSite: "strict", secure: false },
				resave: false
			})
		)
		mockApp.all("*", function (req, res, next) {
			req.session.userid = "huynhnamdo"
			next()
		})
		mockApp.use(app)
	})
	describe("User is not logged in after checking session", () => {
		test("should return json object with a message of 'Log in required'", async () => {
			const response = await request(app).get(`/api/tweet/_id/${TESTING_TWEET_ID}`)

			expect(JSON.parse(response.text)).toEqual({ message: "Log in required" })
		})
	})

	describe("User is logged in after checking session", () => {
		test("should return a json object with a tweet of that id", async () => {
			const response = await request(mockApp).get(`/api/tweet/_id/${TESTING_TWEET_ID}`)

			expect(JSON.parse(response.text).tweet._id).toBe(TESTING_TWEET_ID)
		})
	})

	describe("Passed in a random tweet id that is not in the system", () => {
		test("should return a json object with a message 'Tweet not found'", async () => {
			const response = await request(mockApp).get(`/api/tweet/_id/jaksdhfidklasf`)

			expect(JSON.parse(response.text)).toEqual({ message: "Tweet not found" })
		})
	})
})
